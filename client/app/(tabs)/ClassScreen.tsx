...
export default function ClassScreen() {
  ...
  const [isSearchEmpty, setIsSearchEmpty] =
    useState(true);
  ...
  const classrooms: ClassItem[] | null =
    useAppSelector(
  ...
  let buttonIcon: keyof typeof Ionicons.glyphMap =
    "school";
  ...
  const handleSearch = (text: string) => {
    ...
    const data: ClassItem[] = classes.filter(
      (item) =>
        item.name
          .toLowerCase()
  ...
  const isEnrollValid =
    classId.trim().length >= 6 &&
    classId.trim().length <= 8;
      ...
      {menuVisible && (
        <View className="absolute bottom-28 right-8 items-end">
          ...
          >
            ...
            <View
              className={`${buttonBg} p-2 rounded-xl`}
            >
              <Ionicons
                name={buttonIcon}
                size={24}
                ...
                {role === "STUDENT" && (
                  <View className="gap-5">
                    <TextInput
                      ...
                      maxLength={8}
                      className="bg-white p-5 rounded-3xl border border-border text-lg tracking-[4px]"
                    />
